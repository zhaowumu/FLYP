...
class ExcelService {
    ...
    // 导出任务到Excel
    async exportTasksToExcel(projectId) {
        try {
    ...
    // 导出BUG到Excel
    async exportBugsToExcel(projectId) {
        try {
    ...
    // 从Excel导入任务
    async importTasksFromExcel(fileBuffer, projectId) {
        try {
            const taskRepository = database_1.AppDataSource.getRepository(Task_1.Task);
            const userRepository = database_1.AppDataSource.getRepository(User_1.User);
            // 读取Excel文件
            ...
            for (const row of data) {
                try {
                    ...
                    failed++;
                    errors.push(`行 ${data.indexOf(row) + 1}: ${error.message}`);
                }
            }
    ...
    // 从Excel导入BUG
    async importBugsFromExcel(fileBuffer, projectId) {
        try {
            const bugRepository = database_1.AppDataSource.getRepository(Bug_1.Bug);
            const userRepository = database_1.AppDataSource.getRepository(User_1.User);
            // 读取Excel文件
            ...
            for (const row of data) {
                try {
                    ...
                    const bug = bugRepository.create({
                        ...
                        project: { id: projectId },
                        assignee: assignee ? { id: assignee.id } : null,
                        reporter: reporter ? { id: reporter.id } : null,
                    });
    ...
    // 辅助方法：转换任务优先级
    convertToTaskPriority(priority) {
